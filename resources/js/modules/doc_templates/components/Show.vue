    ...
    <div class="main">
        <nav aria-label="breadcrumb" class="main-breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><router-link to="/doc_templates">Шаблоны документов</router-link></li>
                <li class="breadcrumb-item active" aria-current="page">{{doc_template.name}}</li>
            </ol>
        </nav>
...
<script>
    ...
    export default {
        ...
        mounted () {
            this.mounted = true;
            this.$store.dispatch('getDocTemplate', this.$route.params.id);
            // this.content = this.doc_template.html ? [this.doc_template.html] : [''];
        },
        ...
        computed: {
            ...
            content() {
                return [this?.doc_template?.html ? this?.doc_template?.html : '']
            },
            ...
            menu () {
                return [
                    // Main commands
                    { text: "", title: "Save", icon: "save", class: "text-success", click: this.updateDocTemplate},
                    { text: "", title: "Print", icon: "print", click: () => window.print() },
        ...
        methods: {
            updateDocTemplate() {
                let doc_template = this.doc_template;
                doc_template.html = this.content[0];
                this.$store.dispatch('updateDocTemplate', doc_template)
            },
